...
# Check for remaining Dec-Feb occurrences in January/February
jan_remaining = 0
feb_remaining = 0
...
for entry in data:
    if entry.get('month') == 'January':
        for key in ['task', 'element', 'subElement', 'monthlyTask']:
            if key in entry and ('Dec-Feb' in entry[key] or 'Dec–Feb' in entry[key]):
                jan_remaining += 1
...
print("="*70)
print(f"Remaining 'Dec-Feb' in January: {jan_remaining}")
print(f"Remaining 'Dec-Feb' in February: {feb_remaining}")
if jan_remaining == 0 and feb_remaining == 0:
    print("\n✅ ALL 'Dec-Feb' and '(Dec-Feb)' SUCCESSFULLY REMOVED!")
else:
